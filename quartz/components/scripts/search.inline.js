...
import { normalizeRelativeURLs, resolveRelative } from "../../util/path";
...
function highlightHTML(searchTerm, el) {
    ...
    const createHighlightSpan = (text) => {
        const span = document.createElement("span");
        span.className = "highlight";
        span.textContent = text;
        return span;
    };
    const highlightTextNodes = (node, term) => {
...
document.addEventListener("nav", async (e) => {
    ...
    async function shortcutHandler(e) {
        ...
        if (e.key === "Enter") {
            ...
            if (document.activeElement === searchBar || currentHover !== null) {
                ...
                const secondResult = firstResult?.nextElementSibling;
    ...
    async function displayPreview(el) {
        if (!searchLayout || !enablePreview || !el || !preview)
            return;
        const slug = el.id;
        const innerDiv = await fetchContent(slug).then((contents) => contents.flatMap((el) => [...highlightHTML(currentSearchTerm, el).children]));
...
async function fillDocument(data) {
    ...
    for (const [slug, fileData] of Object.entries(data)) {
        promises.push(index.addAsync(id++, {
            id,
            slug: slug,
