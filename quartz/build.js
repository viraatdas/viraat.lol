...
async function startServing(ctx, mut, initialContent, clientRefresh, dependencies) {
    const { argv } = ctx;
...
async function partialRebuildFromEntrypoint(filepath, action, clientRefresh, buildData) {
    ...
    // UPDATE DEP GRAPH
    const fp = joinSegments(argv.directory, toPosixPath(filepath));
    const staticResources = getStaticResourcesFromPlugins(ctx);
    let processedFiles = [];
    switch (action) {
        case "add":
    ...
    for (const emitter of cfg.plugins.emitters) {
        ...
        if (depGraph.hasNode(fp)) {
            ...
            const upstreams = [...depGraph.getLeafNodeAncestors(fp)];
...
async function rebuildFromEntrypoint(fp, action, clientRefresh, buildData) {
